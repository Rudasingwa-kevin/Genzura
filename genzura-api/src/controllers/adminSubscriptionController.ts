  ...
  static async grantAccess(req: Request, res: Response) {
    try {
      ...
      await AuditService.logUserAction(
        ...
        `Granted ${plan} subscription to ${user.name} for ${durationDays} days. Reason: ${reason || 'Admin action'}`,
        adminUser.id,
        adminUser.name,
        adminUser.role,
        req
  ...
  static async extendSubscription(req: Request, res: Response) {
    try {
      ...
      await AuditService.logUserAction(
        ...
        `Extended subscription for ${user.name} by ${extensionDays} days. New end date: ${newEndDate.toLocaleDateString()}. Reason: ${reason || 'Admin action'}`,
        adminUser.id,
        adminUser.name,
        adminUser.role,
        req
  ...
  static async revokeAccess(req: Request, res: Response) {
    try {
      ...
      await AuditService.logUserAction(
        ...
        `Cancelled subscription for ${user.name} (was: ${user.subscriptionPlan}). Downgraded to free plan. Reason: ${reason || 'Admin action'}`,
        adminUser.id,
        adminUser.name,
        adminUser.role,
        req
