 ...
 */
const mapRouteToAuditAction = (method: string, path: string): AuditAction | null => {
 ...
 */
const generateDescription = (req: Request, action: AuditAction): string => {
 ...
 */
const extractResourceInfo = (req: Request): { resourceType?: string; resourceId?: string } => {
  const path = req.path;
