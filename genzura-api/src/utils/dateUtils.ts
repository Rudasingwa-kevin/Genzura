...
export class DateService {
  private static readonly REASONABLE_PAST_YEARS = 5; // Data older than 5 years is suspicious
  private static readonly REASONABLE_FUTURE_YEARS = 1; // Data more than 1 year in future is suspicious
   ...
   */
  static formatDate(date: Date | string | null | undefined, format: 'short' | 'long' = 'short'): string {
   ...
   */
  static systemClockHealthCheck(): { healthy: boolean; message: string; systemDate: string } {
    const now = new Date();
...
export const validateDate = (date: Date) => DateService.validateDate(date);
export const daysBetween = (start: Date, end: Date) => DateService.daysBetween(start, end);
export const formatDate = (date: Date | string | null | undefined, format?: 'short' | 'long') =>
  DateService.formatDate(date, format);
