 ...
 */
export async function matchLawsToCase(
 ...
 */
async function fetchRelevantLawsByType(caseType: CaseType) {
 ...
 */
function formatLawsForClaude(laws: any[]): string {
  return laws
    .map(
      (article) => `
 ...
 */
export async function saveLawsToCase(
  caseId: string,
  matches: LawMatch[],
  userId?: string
) {
 ...
 */
export async function matchAndSaveLaws(
  caseId: string,
  caseDetails: CaseDetails,
  userId?: string
) {
 ...
 */
export async function getCaseLaws(caseId: string) {
