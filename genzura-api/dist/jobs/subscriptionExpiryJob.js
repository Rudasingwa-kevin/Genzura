...
export class SubscriptionExpiryJob {
     ...
     */
    static async handleExpiredSubscription(user) {
        ...
        await EmailService.sendSubscriptionExpiredEmail(user.email, user.name, previousPlan, casesCount, docsCount, caseOverage, docOverage);
     ...
     */
    static async sendExpiryWarning(user, daysUntil) {
        try {
            await EmailService.sendSubscriptionExpiryWarning(user.email, user.name, user.subscriptionPlan, user.subscriptionEndDate, daysUntil);
     ...
     */
    static async sendGracePeriodWarning(user, daysExpired) {
        try {
            await EmailService.sendGracePeriodWarning(user.email, user.name, user.subscriptionPlan, daysExpired);
...
//# sourceMappingURL=subscriptionExpiryJob.js.map